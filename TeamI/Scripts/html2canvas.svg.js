...
<style>
        elements in document.styleSheets. // Safari 2 also does not support Object.prototype.propertyIsEnumerable. var styleElements = Object.prototype.propertyIsEnumerable ? elementsByTagName('style') : {
            length: 0;
        }

        ; var linkElements = elementsByTagName('link'); DOM.ready(function() {
        ...
        if (foundAll) break; // this is rather unlikely to happen style = this.getStyle(search = search.parentNode);
        }
        return types;
        }
        ...
        this.get = function(style, weight) {
            ...
            // we don't have to worry about "bolder" and "lighter" // because IE's currentStyle returns a numeric value for it, // and other browsers use the computed value anyway weight =;

        {
            normal: 400, bold: 700;
        }

        [weight] || parseInt(weight, 10); if (weights[weight]) return weights[weight]; // http://www.w3.org/TR/CSS21/fonts.html#propdef-font-weight // Gecko uses x99/x01 for lighter/bolder var up = {
            1: 1, 99: 0;
        }

        [weight % 100], alts = [], min, max; if (up === undefined) up = weight > 400; if (weight == 500) weight = 400; for (var alt in weights) {
            alt = parseInt(alt, 10);
        ...
        if (weight < min) weight = min; if (weight > max) weight = max; alts.sort(function(a, b) {
            return (up
        ...
        function trigger(el) {
            // A timeout is needed so that the event can actually "happen" // before replace is triggered. This ensures that styles are up // to date. setTimeout(function() {
        ...
        , key; for (var i = 0, l = arguments.length; i < l; ++i) {
        ...
        {
            // @todo figure out a better way to do this if (/^\s/.test(text)) parts.unshift('');
        ...
        if (!style) style = CSS.getStyle(el).extend(options); if (!font) font = getFont(el, style); if (!font) continue; if (redraw) {
            engines [options.engine](font, null, style, options, node, el);
            continue;
        }

        var text = node.data; //for some reason, the carriage return is not stripped by IE but "\n" is, so let's keep \r as a new line marker... if (typeof G_vmlCanvasManager != 'undefined') {
        ...
        var HAS_BROKEN_REGEXP = ' '.split(/\s+/).length == 0; var sharedStorage = new Storage(); var hoverHandler = new HoverHandler(); var replaceHistory = []; var engines = {
        }
        ...
        , defaultOptions = {
            engine: null, //fontScale: 1, //fontScaling: false, hover: false, hoverables:;
        ...
        separate: 'words', // 'none' and 'characters' are also accepted textShadow: 'none'
        }
        ...
        api.replace = function(elements, options, ignoreHistory) {
            ...
            // there's no browser support so we'll just stop here if (typeof options.textShadow == 'string' && options.textShadow) options.textShadow = CSS.textShadow(options.textShadow);
        ...
        ; api.engines = engines; api.fonts = fonts;
        /** @ignore */
...
(function(global) {
  ...
  /**
   * Add a <g>
    element that envelop all SCG elements and makes the viewbox transformMatrix descend on all elements
    */
        ...
        */
        ...
        ' />\n'
            ...
            */
            ...
            /**
            * Creates an instance of fabric.Path from an SVG <path>
                element
                * @static
                * @memberOf fabric.Path
    ...
    /**
          ...
          (i === 0 || this.useNative ? ' y' : 'dy' ), '="' ,
                            ...
                            // doing this on <tspan>
                                ...
                                // on containing <text>
                                    one doesn't work in Illustrator
